    ...
    def _detect_concept_gaps(self, student_df: pd.DataFrame) -> Dict:
        ...
        for topic in student_df['Topic'].unique():
            ...
            if topic_accuracy < 0.6:
                topic_trend = self._detect_trend_for_topic(topic_data)
                gaps[f'concept_gap_{topic.lower().replace(" ", "_")}'] = {
                    ...
                    'trend': topic_trend # Add trend information
                }
        
        return gaps
    
    def _detect_trend_for_topic(self, topic_df: pd.DataFrame) -> str:
        """
        Detects the accuracy trend for a given topic over time.
        Compares accuracy in early attempts vs. later attempts.
        """
        if len(topic_df) < self.min_attempts_threshold * 2: # Need enough data for trend
            return 'stable'

        # Sort by timestamp to get chronological order
        topic_df_sorted = topic_df.sort_values('Timestamp')

        # Divide into early and late attempts (e.g., first and last third)
        third_length = max(1, len(topic_df_sorted) // 3)
        
        early_attempts = topic_df_sorted.head(third_length)
        late_attempts = topic_df_sorted.tail(third_length)

        early_accuracy = early_attempts['Correct'].mean()
        late_accuracy = late_attempts['Correct'].mean()

        if late_accuracy > early_accuracy * 1.1: # 10% improvement
            return 'improving'
        elif late_accuracy < early_accuracy * 0.9: # 10% decline
            return 'declining'
        else:
            return 'stable'

    def _detect_confidence_gaps(self, student_df: pd.DataFrame) -> Dict:
        """Detect confidence issues through hesitation patterns, potentially considering trends."""
        gaps = {}
