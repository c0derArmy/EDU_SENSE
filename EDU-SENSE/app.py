...
if page == "Dashboard":
    ...
    with col2_data:
        uploaded_file = st.file_uploader("Upload External Student Data (CSV or Excel)", type=["csv", "xlsx", "xls"], key="upload_data_dashboard")
        if uploaded_file is not None:
            ...
            try:
                ...
                st.session_state.analysis_results = None # Clear previous single student analysis
                st.session_state.overall_analysis_results = None # Reset overall analysis
                st.success(f"✅ File '{uploaded_file.name}' loaded successfully! ({len(raw_df)} records)")
    ...
    if st.session_state.data_loaded and not st.session_state.student_data.empty:
        ...
        if st.session_state.analysis_results is not None:
            ...
            for gap_type, details in results['gaps'].items():
                trend_info = f"<br>Trend: <strong>{details['trend'].capitalize()}</strong>" if 'trend' in details else ""
                
                if details['severity'] == 'high':
                    st.markdown(f"""
                    <div class="gap-alert">
                        <strong>🔴 {gap_type.upper().replace('CONCEPT_GAP_', '').replace('_', ' ')}</strong><br>
        ...
        for i, rec in enumerate(recommendations, 1):
            with st.expander(f"🎯 Recommendation {i}: {rec['title']}", expanded=(i==1)):
                ...
                if rec.get('resources'):
                    ...
                    for res_name, res_link in rec['resources'].items():
                        if isinstance(res_link, list):
                            # Handle list of resources
                            st.write(f"**{res_name}:**")
                            for link in res_link:
                                st.markdown(f"  - [{link}]({link})")
                        else:
                            # Handle single resource link
                            st.markdown(f"- [{res_name}]({res_link})")
